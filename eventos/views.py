...
def inscrever_evento(request, id):
    ...
    if request.method == "GET":
        ...
        # todo validar se o usuario ja é um participante
...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        img_final = InMemoryUploadedFile(
            output, 'ImageField', f'{token_urlsafe(8)}.png', 'image/jpeg', sys.getsizeof(output), None)

        certificado_gerado = Certificado(
            certificado=img_final, participante=participante, evento=evento)
...
def procurar_certificado(request,id):
    ...
    if not certificado:
        messages.add_message(request, constants.ERROR,'Esse certificado ainda nao foi gerado')
        return redirect(reverse('certificados_evento', kwargs={'id': evento.id}))
